...
public class RecipeServiceImpl implements RecipeService {
    ...
    public RecipeRecommendResponse getRandomRecipe() {
        ...
        // 이미지 정보 추가
        List<RecipeSourceInfo> mainImages = selectedRecipe.getRecipeSource().stream()
                .filter(source -> source.getRecipeSourceType().getRecipeSourceType().equals("image"))
                .map(source -> {
                    RecipeSourceInfo info = new RecipeSourceInfo();
                    info.setFilePath(source.getRecipeSourceSave());  // URL만 설정
                    info.setOriginalName(source.getRecipeSourceFileName());
                    return info;
                })
                .collect(Collectors.toList());
    ...
    @Override
    @Transactional
    public RecipeLikeResponse getLikeDislike(RecipeLikeRequest request) {
        RecipeLikeDislike likeDislike = recipeLikeDislikeRepository.findByUserUserPkAndRecipeRecipePk(
                request.getUserPk(),
                request.getRecipePk()
        ).orElseThrow(() -> new IllegalStateException("No like/dislike record found"));
        RecipeLikeResponse res = new RecipeLikeResponse(0, 0, likeDislike.getLikeStatus());
        return res;
    }
